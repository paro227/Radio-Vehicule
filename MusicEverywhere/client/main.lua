...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "loop" then
		...
		if type(datasoundinfo.loop) ~= "table" then
			local loop = ('<b>En boucle:</b> '.. firstToUpper(tostring(datasoundinfo.loop)))
			SendNUIMessage({
				action = "changetextl",
				text = loop,
			})
...
function show(nomecenas)
    ...
    elseif nuiaberto then
		...
		ESX.ShowNotification("~r~Tu ne peux pas faire ça maintenant")
	end
end
		...
		for i = 1, #Config.Zones do
			...
			if myjob.name == v.job then
				...
				if distance <= 10 then
					...
					if distance <= 3 then
						dormir = 5
						DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "[~r~E~w~] - Changer de musique")
